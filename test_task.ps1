...
# 1. Create task
Write-Host "[STEP] Creating task: keyword=$KEYWORD, target=$TARGET_COUNT"
$body = @{
    keyword = $KEYWORD
    target_count = $TARGET_COUNT
    task_type = "search_keyword_videocontact"
} | ConvertTo-Json -Compress

$resp = Invoke-RestMethod -Uri "$BASE_URL/api/v1/tasks/start" -Method Post -ContentType "application/json" -Body $body
...
if ($resp.code -eq 0) {
    ...
    if ($d.status -eq "failed") {
        Write-Host ""
        Write-Host "[INFO] Task failed immediately (no browser connected)" -ForegroundColor Yellow
        ...
        try {
            $deleteResp = Invoke-RestMethod -Uri "$BASE_URL/api/v1/tasks/$taskId" -Method Delete
...
try {
    $resp = Invoke-RestMethod -Uri "$BASE_URL/api/v1/tasks/$taskId/pause" -Method Post
    if ($resp.code -eq 0 -and $resp.data.success -eq $true) {
...
try {
    $resp = Invoke-RestMethod -Uri "$BASE_URL/api/v1/tasks/$taskId/resume" -Method Post
    if ($resp.code -eq 0 -and $resp.data.success -eq $true) {
...
try {
    $resp = Invoke-RestMethod -Uri "$BASE_URL/api/v1/tasks/$taskId" -Method Delete
    if ($resp.code -eq 0 -and $resp.data.success -eq $true) {
        Write-Host "[OK] Task deleted" -ForegroundColor Green
    } else {
